...
description: "BotMeriendo uses a cog-based modular architecture: three Discord.py Cog modules are loaded dynamically at startup through a single entry point with shared in-memory state."
---
...
<CardGroup cols={3}>
  ...
  <Card title="src/utils/state.py" icon="database">
    Defines the shared `music_queues` dict (keyed by guild ID) that every music-related module imports. Single source of truth for per-guild queue state.
  </Card>
...
`music_queues` is a plain Python `dict` keyed by guild ID (`int`). Each value is a list of `(video_url, title)` tuples where `video_url` is the canonical YouTube watch URL and `title` is the human-readable video name. Both `src/cogs/music.py` and `src/cogs/events.py` import this same object, so mutations from either cog are immediately visible to the other.
